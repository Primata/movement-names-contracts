...
module movement_names::config {
    friend movement_names::domains;
    ...
    const CONFIG_OBJECT_SEED: vector<u8> = b"MNS v2 config";
    ...
    public fun is_enabled(): bool acquires Config {
        borrow_global<Config>(@movement_names).enabled
    }

    #[view]
    public fun fund_destination_address(): address acquires Config {
        borrow_global<Config>(@movement_names).fund_destination_address
    }

    #[view]
    public fun admin_address(): address acquires Config {
        borrow_global<Config>(@movement_names).admin_address
    }

    #[view]
    public fun max_number_of_seconds_registered(): u64 acquires Config {
        borrow_global<Config>(@movement_names).max_number_of_seconds_registered
    }

    #[view]
    public fun max_domain_length(): u64 acquires Config {
        borrow_global<Config>(@movement_names).max_domain_length
    }

    #[view]
    public fun min_domain_length(): u64 acquires Config {
        borrow_global<Config>(@movement_names).min_domain_length
    }

    #[view]
    public fun tokendata_description(): String acquires Config {
        borrow_global<Config>(@movement_names).tokendata_description
    }

    #[view]
    public fun tokendata_url_prefix(): String acquires Config {
        borrow_global<Config>(@movement_names).tokendata_url_prefix
    }
    ...
    public fun domain_price_for_length(domain_length: u64): u64 acquires Config {
        ...
        if (domain_length == 3) {
            borrow_global<Config>(@movement_names).domain_price_length_3
        } else if (domain_length == 4) {
            borrow_global<Config>(@movement_names).domain_price_length_4
        } else if (domain_length == 5) {
            borrow_global<Config>(@movement_names).domain_price_length_5
        } else {
            borrow_global<Config>(@movement_names).domain_price_length_6_and_above
        }
    }

    #[view]
    public fun subdomain_price(): u64 acquires Config {
        borrow_global<Config>(@movement_names).subdomain_price
    }

    #[view]
    public fun reregistration_grace_sec(): u64 acquires Config {
        borrow_global<Config>(@movement_names).reregistration_grace_sec
    }
    ...
    public fun signer_is_admin(sign: &signer): bool acquires Config {
        signer::address_of(sign) == admin_address() || signer::address_of(sign) == @movement_names
    }
    ...
    public entry fun set_is_enabled(sign: &signer, enabled: bool) acquires Config {
        ...
        borrow_global_mut<Config>(@movement_names).enabled = enabled
    }
    ...
    public entry fun set_fund_destination_address(sign: &signer, addr: address) acquires Config {
        ...
        borrow_global_mut<Config>(@movement_names).fund_destination_address = addr
    }
    ...
    public entry fun set_admin_address(sign: &signer, addr: address) acquires Config {
        ...
        borrow_global_mut<Config>(@movement_names).admin_address = addr
    }
    ...
    public entry fun set_max_number_of_seconds_registered(sign: &signer, max_seconds_registered: u64) acquires Config {
        ...
        borrow_global_mut<Config>(@movement_names).max_number_of_seconds_registered = max_seconds_registered
    }
    ...
    public entry fun set_max_domain_length(sign: &signer, domain_length: u64) acquires Config {
        ...
        borrow_global_mut<Config>(@movement_names).max_domain_length = domain_length
    }

    public entry fun set_min_domain_length(sign: &signer, domain_length: u64) acquires Config {
        assert_signer_is_admin(sign);
        assert!(domain_length > 0, error::invalid_argument(EINVALID_VALUE));
        borrow_global_mut<Config>(@movement_names).min_domain_length = domain_length
    }

    public entry fun set_tokendata_description(sign: &signer, description: String) acquires Config {
        assert_signer_is_admin(sign);
        borrow_global_mut<Config>(@movement_names).tokendata_description = description
    }

    public entry fun set_tokendata_url_prefix(sign: &signer, url_prefix: String) acquires Config {
        assert_signer_is_admin(sign);
        borrow_global_mut<Config>(@movement_names).tokendata_url_prefix = url_prefix
    }

    public entry fun set_subdomain_price(sign: &signer, price: u64) acquires Config {
        assert_signer_is_admin(sign);
        borrow_global_mut<Config>(@movement_names).subdomain_price = price
    }
    ...
    public entry fun set_domain_price_for_length(sign: &signer, price: u64, length: u64) acquires Config {
        ...
        if (length == 3) {
            borrow_global_mut<Config>(@movement_names).domain_price_length_3 = price
        } else if (length == 4) {
            borrow_global_mut<Config>(@movement_names).domain_price_length_4 = price
        } else if (length == 5) {
            borrow_global_mut<Config>(@movement_names).domain_price_length_5 = price
        } else {
            borrow_global_mut<Config>(@movement_names).domain_price_length_6_and_above = price
        }
    }

    public entry fun set_reregistration_grace_sec(sign: &signer, reregistration_grace_sec: u64) acquires Config {
        assert_signer_is_admin(sign);
        borrow_global_mut<Config>(@movement_names).reregistration_grace_sec = reregistration_grace_sec
    }
    ...
    #[test_only]
    friend movement_names::price_model;
    #[test_only]
    ...
    #[test_only]
    public fun initialize_for_test(movement_names: &signer, aptos: &signer) acquires Config {
        timestamp::set_time_has_started_for_testing(aptos);
        initialize_aptoscoin_for(aptos);
        initialize_config(movement_names, @movement_names, @movement_names);
        set_admin_address_test_only(@movement_names);
    }

    #[test(myself = @movement_names)]
    fun test_default_token_configs_are_set(myself: signer) acquires Config {
        ...
        initialize_config(&myself, @movement_names, @movement_names);
        set_fund_destination_address_test_only(@movement_names);
    ...
    #[test(myself = @movement_names, rando = @0x266f, aptos = @0x1)]
