#[test_only]
module movement_names::test_helper {
    ...
    public fun register_name(
        ...
        let is_owner = domains::is_token_owner(user_addr, domain_name, subdomain_name);
        let is_expired = domains::is_name_expired(domain_name, subdomain_name);
        ...
        if (is_subdomain) {
            ...
            assert!(domain_price / config::octas() == 10, domain_price / config::octas());
        ...
        let expiration_time_sec = domains::get_expiration(domain_name, subdomain_name);
    ...
    public fun set_target_address(
        ...
        let target_address = domains::get_target_address(domain_name, subdomain_name);
        test_utils::print_actual_expected(
        ...
        if (option::is_some(&maybe_reverse_lookup_before)) {
            let (maybe_reverse_subdomain, reverse_domain) = domains::get_name_props_from_token_addr(
