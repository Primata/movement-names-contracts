module movement_names::token_helper {
    ...
    public(friend) fun get_fully_qualified_domain_name(subdomain_name: Option<String>, domain_name: String): String {
        let (domain_is_allowed, _length) = string_validator::string_is_allowed(&domain_name);
        ...
        let subdomain_is_allowed = if (option::is_some(&subdomain_name)) {
            let (subdomain_is_allowed, _length) = string_validator::string_is_allowed(option::borrow(&subdomain_name));
