...
module router::primary_name_tests {
    ...
    inline fun get_v1_primary_name(
        ...
        let record = movement_names::domains::get_reverse_lookup(user_addr);
        if (option::is_none(&record)) {
            (option::none(), option::none())
        } else {
            let (subdomain_name, domain_name) = movement_names::domains::get_name_record_key_v1_props(
    ...
    inline fun exists_in_v2(domain_name: String, subdomain_name: Option<String>): bool {
        object::is_object(movement_names::domains::get_token_addr(domain_name, subdomain_name))
    }

    inline fun get_v2_primary_name(
        user_addr: address
    ): (Option<String>, Option<String>) {
        let token_addr = movement_names::domains::get_reverse_lookup(user_addr);
        if (option::is_none(&token_addr)) {
            (option::none(), option::none())
        } else {
            let (subdomain_name, domain_name) = movement_names::domains::get_name_props_from_token_addr(
    ...
    fun test_set_primary_name_should_trigger_auto_migration(
        ...
        router::set_primary_name(user, domain_name2, option::none());
        {
            ...
            assert!(movement_names::domains::is_token_owner(user_addr, domain_name2, option::none()), 2);
            assert!(!movement_names::domains::is_name_expired(domain_name2, option::none()), 3);
    ...
    fun test_clear_domain_primary_name_should_trigger_auto_migration(
        ...
        router::clear_primary_name(user);
        {
            ...
            let (is_owner_of_v1_name, _) = movement_names::domains::is_token_owner(user_addr, option::none(), domain_name);
            assert!(!is_owner_of_v1_name, 1);
            assert!(movement_names::domains::is_token_owner(user_addr, domain_name, option::none()), 2);
            assert!(!movement_names::domains::is_name_expired(domain_name, option::none()), 2);
            // v1 primary name should be cleared
    ...
    fun test_clear_subdomain_primary_name_should_not_trigger_auto_migration(
        ...
        router::clear_primary_name(user);
        {
            ...
            let (is_owner_of_v1_name, _) = movement_names::domains::is_token_owner(user_addr, subdomain_name_opt, domain_name);
            assert!(!movement_names::domains::name_is_expired(subdomain_name_opt, domain_name), 1);
            assert!(is_owner_of_v1_name, 1);
            assert!(!movement_names::domains::is_token_owner(user_addr, domain_name, subdomain_name_opt), 2);
            assert!(movement_names::domains::is_name_expired(domain_name, subdomain_name_opt), 2);
            // v1 primary name should be cleared
            let (v1_primary_subdomain_name, v1_primary_domain_name) = get_v1_primary_name(user_addr);
            assert!(option::is_none(&v1_primary_domain_name), 2);
